...
if (isset($_GET['name'])) {
    ...
    $count_query = $pdo->prepare("SELECT COUNT(*) FROM $table1 INNER JOIN $table2 ON products.id_cat = categories.id WHERE categories.name = :name");
    $params1 = [':name' => $name];
    $count_query->execute($params1);
    ...
    if (isset($_GET['do_new']) || $_GET['do_sorting'] == 'do_new') {
        ...
        $stmt = $pdo->prepare("SELECT categories.name AS name_cat, categories.description AS des_cat, categories.img_cat, products.id, products.name, products.description, products.price, products.discount, products.img  FROM $table1 INNER JOIN $table2 ON products.id_cat = categories.id WHERE categories.name = ? ORDER BY id DESC LIMIT ?, ?");
    } elseif (isset($_GET['do_old']) || $_GET['do_sorting'] == 'do_old') {

        $do_sorting = 'do_old';

        $stmt = $pdo->prepare("SELECT categories.name AS name_cat, categories.description AS des_cat, categories.img_cat, products.id, products.name, products.description, products.price, products.discount, products.img  FROM $table1 INNER JOIN $table2 ON products.id_cat = categories.id WHERE categories.name = ? ORDER BY id ASC LIMIT ?, ?");
    } elseif (isset($_GET['do_price']) || $_GET['do_sorting'] == 'do_price') {

        $do_sorting = 'do_price';

        $stmt = $pdo->prepare("SELECT categories.name AS name_cat, categories.description AS des_cat, categories.img_cat, products.id, products.name, products.description, products.price, products.discount, products.img  FROM $table1 INNER JOIN $table2 ON products.id_cat = categories.id WHERE categories.name = ? ORDER BY price DESC LIMIT ?, ?");
    } else {
    ...
    function Pagination($length, $page, $name)
    {
        global $do_sorting;

        foreach (range(1, $length) as $p) {
            if ($page == $p) {
                echo '<li class="page-item active"><a class="page-link" href="http://shop/category.php/' . '?name=' . $name . '&page=' . $p . '&do_sorting=' . $do_sorting . '">' . $p . '</a></li>';
            } else {
                echo
                    '<li class="page-item"><a class="page-link" href="http://shop/category.php/' . '?name=' . $name . '&page=' . $p . '&do_sorting=' . $do_sorting . '">' . $p .  '</a></li>';
            }
        }
    }
}
        ...
        <div class="container">
            <div class="row align-items-center my-5">
                <?php foreach ($category as $row) : ?>
                    <div class="col-lg-7">
                        <center>
                            <img style="max-width: 65%;" class="img-fluid rounded mb-4 mb-lg-0" src="/img/<?= show_img($row['img_cat']); ?>" alt="">
                        </center>
                    </div>
                    <div class="col-lg-5">
                        <h1 class="font-weight-light"><?= $row['name_cat'] ?></h1>
                        <p><?= $row['des_cat'] ?></p>
                    </div>
                    <?php break; ?>
                <?php endforeach; ?>
            ...
            <div class="row">
                ...
                <div class="col-md-3 order-md-2 mb-4">
                    ...
                    <?php if (isset($_GET['success_promo'])) : ?>
                        ...
                        <hr>
                    <?php endif; ?>
                    <form class="card p-2" action="/discounts.php/?<?= "name=$name&page=$page" ?>" method="post">
                        <div class="input-group">
                            <input type="text" name="promo" class="form-control" placeholder="Промокод">
                            <div class="input-group-append">
                                <button type="submit" name="do_category_promo" class="btn btn-secondary">Отправить</button>
                            </div>
                        </div>
                    </form>
                </div>
            ...
            <nav aria-label="Search results pages">
                <ul class="pagination">
                    <li class="page-item"><a class="page-link" href="<?= "http://shop/category.php/?name=$name&page=$prev&do_sorting=$do_sorting"; ?>">Назад</a></li>
                    <?= Pagination($length, $page, $name); ?>
                    <li class="page-item"><a class="page-link" href="<?= "http://shop/category.php/?name=$name&page=$next&do_sorting=$do_sorting"; ?>">Дальше</a></li>
                </ul>
            </nav>
        </div>
    </main>
