...
$table1 = "products";
$table2 = "categories";
$count_query = $pdo->query("SELECT COUNT(*) FROM $table1");
...
if (isset($_GET['do_new']) || $_GET['do_sorting'] == 'do_new') {
  ...
  $stmt = $pdo->prepare("SELECT categories.name AS name_cat, products.id, products.name, products.description, products.price, products.discount, products.img  FROM $table1 INNER JOIN $table2 ON products.id_cat = categories.id ORDER BY id DESC LIMIT ?, ?");

} elseif (isset($_GET['do_old']) || $_GET['do_sorting'] == 'do_old') {

  $do_sorting = 'do_old';

  $stmt = $pdo->prepare("SELECT categories.name AS name_cat, products.id, products.name, products.description, products.price, products.discount, products.img  FROM $table1 INNER JOIN $table2 ON products.id_cat = categories.id ORDER BY id ASC LIMIT ?, ?");

} elseif (isset($_GET['do_price']) || $_GET['do_sorting'] == 'do_price') {

  $do_sorting = 'do_price';

  $stmt = $pdo->prepare("SELECT categories.name AS name_cat, products.id, products.name, products.description, products.price, products.discount, products.img  FROM $table1 INNER JOIN $table2 ON products.id_cat = categories.id ORDER BY price DESC LIMIT ?, ?");
...
function Pagination($length, $page)
{
  ...
  foreach (range(1, $length) as $p) {
    if ($page == $p) {
      echo '<li class="page-item active"><a class="page-link" href="http://shop/?do_sorting=' . $do_sorting . '&page=' . $p . '">' . $p . '</a></li>';
    } else {
      echo '<li class="page-item"><a class="page-link" href="http://shop/?do_sorting=' . $do_sorting . '&page=' . $p . '">' . $p . '</a></li>';
    }
  }
}
  ...
  <main role="main" class="flex-shrink-0">
    <div class="container">
      <div class="row">
        <div class="col-lg-3">
          <center>
            <h1 class="my-4">Магазин <i>#Самовар.ру</i></h1>
          </center>
          <div class="list-group">
            <?php
            $sql_cat = $pdo->query("SELECT name FROM $table2");
            $cats = $sql_cat->fetchAll(PDO::FETCH_ASSOC);
            foreach ($cats as $cat) {
              echo "<h4><a style='color:black;' href='/category.php?name=$cat[name]' class='list-group-item'>$cat[name]</a></h4>";
            }
            /*
          $elements = [];
          foreach ($rows as $row) {

            if (in_array($row['name_cat'], $elements)) {
              continue;
            } else {
              $elements[] = $row['name_cat'];
            }
            echo "<a href='/category.php' class='list-group-item'>$row[name_cat]</a>";
          }
          */
            ?>
          </div>
          ...
          <div id="carouselControls" class="carousel slide" data-ride="carousel">
            <div class="carousel-inner">
              <div class="carousel-item active">
                <img src="/img/samovar-rospis-russkii-stil-baranki-khleb-chaepitie-chai-cha.jpg" class="d-block w-100 img-thumbnail" alt="...">
              </div>
              <div class="carousel-item">
                <img src="/img/samovar-chai-stol-russkii-stil-vypechka-chainik-podnos-korzi.jpg" class="d-block w-100 img-thumbnail" alt="...">
              </div>
            </div>
    ...
    <div class="container pt-3">
      ...
      <div class="row">
              ...
              <div class="col-lg-4 col-md-6 mb-4">
                <div class="card h-100">
                  ...
                  <div class="card-body">
                    <hr>
                    <h4 class="card-title">
                      <a style='color:black;' href="/category.php?name=<?= $row['name_cat']; ?>"><?= $row['name_cat']; ?></a>
                    </h4>
                    <hr>
                    <h4 class="card-title">
                      <a style='color:black;' href="/details.php?id=<?= $row['id']; ?>"><?= $row['name']; ?></a>
                    </h4>
                    <hr>
        ...
        <div class="col-md-3 order-md-2 mb-4">
          ...
          <?php if (isset($_GET['success_promo'])) : ?>
            ...
            <hr>
          <?php endif; ?>
          <form class="card p-2" action="/discounts.php/?page=<?= $page; ?>" method="post">
            <div class="input-group">
              <input type="text" name="promo" class="form-control" placeholder="Промокод">
              <div class="input-group-append">
                <button type="submit" name="do_index_promo" class="btn btn-secondary">Отправить</button>
              </div>
            </div>
          </form>
        </div>
      ...
      <nav aria-label="Search results pages">
        <ul class="pagination">
          <li class="page-item"><a class="page-link" href="<?= "http://shop/?do_sorting=$do_sorting&page=$prev"; ?>">Назад</a></li>
          <?= Pagination($length, $page); ?>
          <li class="page-item"><a class="page-link" href="<?= "http://shop/?do_sorting=$do_sorting&page=$next"; ?>">Дальше</a></li>
        </ul>
      </nav>
    </div>
  </main>
